...
from keras.layers import Conv2D, Input, Reshape, Lambda, BatchNormalization, MaxPooling2D, Multiply,GlobalMaxPooling1D, Concatenate, Dense
import numpy as np
import keras
...
#load images, labels, clinical data, and probs from stage one
x_train = np.load('path to images').astype('float32')
y_train=np.load('path to labels')
prob_train=np.load('path to probs')
clin_train=np.load('path to clinical data').astype('float')

#do not forget to standardize weight and age
...
clin_added=Concatenate()([final_outputs, input_shape3])
final2=Dense(128, activation='relu')(clin_added)
final2=Dense(64, activation='relu')(final2)
final2=Dense(32, activation='relu')(final2)
final2=Dense(3, activation='sigmoid')(final2)



#Define model
model = Model(inputs=[input_shape,input_shape2,input_shape3], outputs=[final2])
...
#train
model.fit([x_train,prob_train,clin_train],[y_train], batch_size = 8, epochs = 150, validation_data = ([x_valid,prob_valid,clin_valid],[y_valid]),callbacks=callbacks_list,shuffle=True,class_weight=d_class_weights)
